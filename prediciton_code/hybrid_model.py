...
class HybridTransformerLSTM(nn.Module):
    ...
    Architecture:
      ...
      - Positional/time embedding (optional via simple linear)
    ...
    """
    def __init__(self, enc_in, d_model=64, nhead=4, num_layers=2, lstm_hidden=128, lstm_layers=1, pred_len=5, dropout=0.05):
        super().__init__()
        self.enc_in = enc_in
        ...
        # simple learnable time embedding (seq_len-independent)
    ...
    def forward(self, enc_inp):
        ...
        # optional time embedding: add simple scaled position number
        pos = torch.arange(seq_len, device=x.device).float().unsqueeze(0).unsqueeze(-1)  # [1, seq_len, 1]
        pos_emb = self.time_proj(pos)  # [1, seq_len, d_model]
        x = x + pos_emb
        # Transformer over time
        x_enc = self.transformer(x)  # [b, seq_len, d_model]
        # take last time step as seed for decoder (can be enhanced to use other pooling)
        dec_input = x_enc[:, -1:, :]  # [b, 1, d_model]
        # initialize LSTM hidden with zeros
        ...
        outputs = []
        hx = (h0, c0)
        cur_input = dec_input  # start from last encoder output
        for _ in range(self.pred_len):
            out, hx = self.lstm(cur_input, hx)  # out: [b,1,lstm_hidden]
            ...
            next_in = self.input_proj(frame.unsqueeze(1))  # [b,1,d_model]
            cur_input = next_in
        out_seq = torch.cat(outputs, dim=1)  # [b, pred_len, enc_in]
        return out_seq
...
# instantiate existing models (informer, lstm, etc.) as before
...
# if you have a pretrained transformer encoder weights (from informer), you could try to load them into hybrid.
# That requires matching parameter names / shapes and careful mapping; left commented for now.
# Example (experimental, may need adaptation):
# try:
#     informer_state = torch.load(<path_to_informer_encoder_only>)
#     # map and load relevant encoder layers into hybrid.transformer
# except Exception as e:
#     print("Skipping hybrid init from informer:", e)

# load pretrained checkpoints (same as your original script)
...
# prepare result arrays (extend with Hybrid)
...
Rate_hybrid = np.zeros(args.pred_len * args.ir_test + 1)
...
NMSE_hybrid = np.zeros(args.pred_len * args.ir_test + 1)
...
with torch.no_grad():
    for it in range(N_it):
        ...
        # hybrid - forward pass (expects enc_inp shape [b, seq_len, enc_in])
        ...
        # reshape outputs to [M, pred_len, Nr, Nt]
        ...
        # LSTM, GRU, RNN, AR, Delay AR, Previous - same as original script
        for s in range(args.pred_len + 1):
            H_true = label[:, s - 1, :, :] if s > 0 else data[:, -args.pred_len - 1, ...]
...
# save existing mats (same as your original script)
